...
#この定義がaに依存していないのでダメ。
mean=function(a) (a^(1/a))*(gamma(2/a)/gamma(1/a))

var=function(a) (a^(2/a))*(gamma(3/a)/gamma(1/a))-((a^(1/a))*(gamma(2/a)/gamma(1/a)))^2

GaussianMixture = function(x){
	(1/sqrt(2*pi*(var(a))^2))*(exp(-((x-mean(a))^2)/2))
...
for( a in 20:1000){
	a = a*0.1
	plot(xi,GaussianMixture(xi),xlab="xi",ylab="GaussianMixture",xlim=c(-4,4),ylim=c(0,1),type="l" ,col=a*10)
	par(new=T)
}
