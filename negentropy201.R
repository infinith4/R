alpha=1
p=function(alpha){
	exp(-(1/2)*abs(xi)^alpha)/(2^(alpha+1)*(gamma(alpha+1)))
}
f = function(xi){
	-p(xi)*log(p(xi))
...
h=function(alpha){
	integrate(f,-Inf,Inf)$val
}
h(1)

J=function(alpha){
	(1+(1/2)*(log(2*pi)))-h(alpha)
}

for(alpha in 2001:3000){
	alpha=alpha*0.001
	plot(alpha,J(alpha),xlab="alpha",ylab="negentropy",xlim=c(0,1),ylim=c(0,5),type="p")
	par(new=T)
}
...
Jk=function(a){
	(1/48)*(gamma(5/a)*gamma(1/a)/(gamma(3/a))^2)^2-0.75
}
...
k=function(r){
	gamma(5*r/2)*gamma(r/2)/(gamma(3*r/2))^2
}

k(1)

a=seq(0.5,3,length=1000)

plot(a,Jk(a),xlab="a",ylab="negentropy",xlim=c(0.5,3),ylim=c(0,2),type="l")
plot(a,Jk(a),xlab="a",ylab="negentropy",xlim=c(2,3),ylim=c(0,0.2),type="l")
...
plot(mu,Ja(mu),xlab="mu",ylab="negentropy",xlim=c(0,1),ylim=c(0,2.5),type="l")

par(new=T)

plot(mu,Jb(mu),xlab="mu",ylab="negentropy",xlim=c(0,1),ylim=c(0,2.5),type="l")
